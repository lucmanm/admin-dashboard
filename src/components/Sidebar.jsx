...
const Sidebar = () => {
  ...
  return (
    <div className="ml-3 h-screen md:overflow-hidden overflow-auto md:hover:overflow-auto pb-10">
      {/* Sidebar Top Menu true or false */}
      {activeMenu && (
        <>
          ...
          {/* List Menus imported from Dummy JS*/}
          <div>
            {/* Dashboard, PAges, Apps, Charts Looping here */}
            {links.map((linksTitle) => (
              <div key={linksTitle.title}>
                <p className="text-gray-400 m-3 mt-4 uppercase">
                  {linksTitle.title}
                </p>
                {/* Sub Categories Loops here. */}
                {linksTitle.links.map((link) => (
                  <NavLink
                    to={`/${link.name}`}
                    key={link.name}
                    onClick={handleCloseSideBar}
